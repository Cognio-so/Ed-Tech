...
export async function getStudentHistory(
  ...
  try {
    ...
    const [conversations, total] = await Promise.all([
      (prisma as any).studentConversation.findMany({
        where: whereClause,
        orderBy: {
          updatedAt: "desc",
        },
      ...
      (prisma as any).studentConversation.count({
        where: whereClause,
      }),
    ]);
...
export async function getStudentConversationById(
  ...
  try {
    const conversation = await (prisma as any).studentConversation.findFirst({
      where: {
        id: conversationId,
        userId: session.user.id,
      },
