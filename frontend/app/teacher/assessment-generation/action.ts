...
export async function saveAssessment(formData: FormData) {
  ...
  try {
    ...
    const confidenceLevel = parseInt(formData.get("confidenceLevel") as string);
    ...
    const trueFalseCount =
      parseInt(formData.get("trueFalseCount") as string) || 0;
    const shortAnswerEnabled = formData.get("shortAnswerEnabled") === "true";
    const shortAnswerCount =
      parseInt(formData.get("shortAnswerCount") as string) || 0;
