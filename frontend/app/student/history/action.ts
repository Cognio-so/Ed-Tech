...
export async function saveStudentConversation(
  messages: string,
  title?: string,
  ...
  try {
    ...
    if (conversationId) {
      const existing = await (prisma as any).studentConversation.findUnique({
        where: { id: conversationId },
      });

      if (existing && existing.userId === session.user.id) {
        await (prisma as any).studentConversation.update({
          where: { id: conversationId },
          data: {
    ...
    const newConversation = await (prisma as any).studentConversation.create({
      data: {
        userId: session.user.id,
        title: conversationTitle,
        messages,
    ...
    console.error("Error saving student conversation:", error);
...
export async function deleteStudentConversation(conversationId: string) {
  ...
  try {
    const conversation = await (prisma as any).studentConversation.findUnique({
      where: { id: conversationId },
    });
    ...
    await (prisma as any).studentConversation.delete({
      where: { id: conversationId },
    });
    ...
    console.error("Error deleting student conversation:", error);
